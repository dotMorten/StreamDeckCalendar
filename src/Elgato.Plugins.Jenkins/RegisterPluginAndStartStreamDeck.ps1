...
$pluginProjectFile = "$basePath\Elgato.Plugins.Jenkins.csproj"
...
$bindir = "$basePath\bin\Debug\Elgato.Plugins.Jenkins.sdPlugin"

# Make sure we actually have a directory/build to deploy
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"Elgato.Plugins.Jenkins`" project before calling this script.";
  exit 1;
}
