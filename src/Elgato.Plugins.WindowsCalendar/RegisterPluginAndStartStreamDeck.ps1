...
$pluginProjectFile = "$basePath\Elgato.Plugins.WindowsCalendar.csproj"
...
$bindir = "$basePath\bin\Debug\Elgato.Plugins.WindowsCalendar.sdPlugin"

# Make sure we actually have a directory/build to deploy
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"Elgato.Plugins.WindowsCalendar`" project before calling this script.";
  exit 1;
}
...
Write-Host "Deployment complete to $destDir. Restarting the Stream Deck desktop application..."
